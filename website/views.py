...
def delete_post(id):
    ...
    if not post:
        ...
        flash("You don not have permission to delete this post", category="error")
    else:
...
def posts(username):
    ...
    if not user:
        flash("No user exists in that username", category="error")
    else:   
        posts = user.post
...
def create_comment(id):
    ...
    if not text:
        ...
        return redirect(url_for("views.home"))
    post = Post.query.filter_by(id=int(id))
    if not post:
        flash("No such post exists!!", category="error")
    else:
        comment = Comment(text=text, author=current_user.id, post_id=int(id))
